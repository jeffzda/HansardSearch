"""
09b_divisions_house.py — Extract division (vote) records from House of Representatives daily XML files.

Produces a supplementary dataset with one row per member per division,
...
Usage:
    python 09b_divisions_house.py --xml-dir ../data/raw/house
...
def _extract_divisions_v21(root, date_str: str) -> tuple[list[dict], list[dict]]:
    ...
    For v2.1, the main chamber is <chamber.xscript> (fedchamb_flag=0) and
    the Federation Chamber is <maincomm.xscript> (fedchamb_flag=1).
    ...
    # Map xscript section tag → fedchamb_flag
    # v2.1: chamber.xscript = main (0), maincomm.xscript = federation chamber (1)
    section_flag_map = {
        "chamber.xscript": 0,
        "maincomm.xscript": 1,
    }

    # Identify top-level xscript sections; if none found, treat entire root as main chamber
    sections = []
    for tag, flag in section_flag_map.items():
        for sec in root.iter(tag):
            sections.append((sec, flag))

    if not sections:
        # No explicit section tags — treat entire document as main chamber
        sections = [(root, 0)]

    for section_el, fedchamb_flag in sections:
        div_no_offset = len(division_rows)
        div_no = div_no_offset

        for div in section_el.iter("division"):
            div_no += 1
...
def _extract_divisions_v22(root, date_str: str) -> tuple[list[dict], list[dict]]:
    ...
    For v2.2, the main chamber is <chamber.xscript> (fedchamb_flag=0) and
    the Federation Chamber is <fedchamb.xscript> (fedchamb_flag=1).
    ...
    # Map xscript section tag → fedchamb_flag
    # v2.2: chamber.xscript = main (0), fedchamb.xscript = federation chamber (1)
    section_flag_map = {
        "chamber.xscript": 0,
        "fedchamb.xscript": 1,
    }
...
# ── Enrichment: join member details from daily parquet ────────────────────────

def _enrich_votes(vote_df: pd.DataFrame, daily_dir: Path,
                  date_str: str) -> pd.DataFrame:
    """Add unique_id, electorate, party from the daily parquet for this date."""
    daily_pq = daily_dir / f"{date_str}.parquet"
    if not daily_pq.exists() or vote_df.empty:
        for col in ("unique_id", "electorate", "party"):
            if col not in vote_df.columns:
                vote_df[col] = None
        return vote_df

    daily = pd.read_parquet(daily_pq, columns=["name_id", "unique_id", "electorate", "party"])
    daily = (
        daily[daily["name_id"].notna()]
        .drop_duplicates("name_id")
        [["name_id", "unique_id", "electorate", "party"]]
    )
...
def extract_divisions(xml_path: Path,
    ...
    _empty_div = pd.DataFrame(columns=[
        ...
        "ayes_count", "noes_count", "result", "fedchamb_flag",
    ])
    _empty_vote = pd.DataFrame(columns=[
        "date", "division_no", "name_id", "name",
        "vote", "unique_id", "electorate", "party",
    ])
...
def _resolve_xml_dir(xml_dir: Path) -> Path:
    """Resolve xml_dir, falling back to sibling 'reps' directory if 'house' doesn't exist."""
    if xml_dir.exists():
        return xml_dir
    # Common case: user specified ../data/raw/house but data lives in ../data/raw/reps
    reps_fallback = xml_dir.parent / "reps"
    if reps_fallback.exists():
        print(f"Note: {xml_dir} not found; using {reps_fallback} instead.")
        return reps_fallback
    return xml_dir  # Let the caller handle the missing directory
...
def extract_all(xml_dir: Path, daily_dir: Path | None,
    ...
    xml_dir = _resolve_xml_dir(xml_dir)
    ...
    if all_divs:
        ...
        combined_divs.to_parquet(out_dir / "house_divisions.parquet", index=False)
        combined_divs.to_csv(out_dir / "house_divisions.csv", index=False)
        print(f"\nDivision summary: {len(combined_divs):,} divisions")
        if "fedchamb_flag" in combined_divs.columns:
            breakdown = combined_divs["fedchamb_flag"].value_counts().sort_index()
            for flag, count in breakdown.items():
                label = "Main chamber" if flag == 0 else "Federation Chamber"
                print(f"  fedchamb_flag={flag} ({label}): {count:,}")

    if all_votes:
        combined_votes = pd.concat(all_votes, ignore_index=True)
        combined_votes.to_parquet(out_dir / "house_division_votes.parquet", index=False)
        combined_votes.to_csv(out_dir / "house_division_votes.csv", index=False)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract House of Representatives Hansard division vote records."
    )
    parser.add_argument("--xml-dir", default="../data/raw/house")
    parser.add_argument("--daily-dir", default="../data/output/house/daily",
                        help="Daily parquet dir for member detail enrichment")
    parser.add_argument("--out-dir", default="../data/output/house/divisions")
